...
class TestSourceYUV420Tests {
    ...
    fun testFormatsAndResolutions() {
        ...
        Assertions.assertEquals(30.0f, resolutions[0].fps)
        Assertions.assertEquals(PixelFormat.YUV420P, resolutions[0].format)
    ...
    fun testCapture() {
        ...
        val source = TestSourceYUV420(font)
        source.open(Source.OpenConfiguration("test"))
        source.startProducing(ProducingConfiguration(
            tag = "testProducing",
            640,
            480,
            30.0f
        ...
        Assertions.assertEquals(FrameFormat.YUV420P.ordinal, frame.getFormat())
