...
public:
    ...
    virtual uint32_t getMiddleRgb(const auvc::Frame &frame, 
        uint16_t x, uint16_t y) const = 0;
    virtual void setMiddleRgb(auvc::Frame &frame, 
        uint16_t x, uint16_t y, uint32_t rgb) const = 0;
    virtual uint16_t getDataSize(const auvc::Frame &frame, 
        uint16_t x, uint16_t y) const;
    virtual void readData(const auvc::Frame &frame, 
...
public:
    ...
    uint32_t getMiddleRgb(const auvc::Frame &frame, uint16_t x, uint16_t y) const override;
    void setMiddleRgb(auvc::Frame &frame, uint16_t x, uint16_t y, uint32_t rgb) const override;
};
